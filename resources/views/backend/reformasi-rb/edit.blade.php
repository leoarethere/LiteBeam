<x-dashboard-layout>
    <x-slot:title>Edit Reformasi RB</x-slot:title>
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-8 pt-2">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Perbarui informasi Reformasi Birokrasi.
                </p>
            </div>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            <form action="{{ route('dashboard.reformasi-rb.update', $reformasiRb->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Judul Reformasi RB <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="title" value="{{ old('title', $reformasiRb->title) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Link File / Dokumen <span class="text-red-500">*</span>
                            </label>
                            @if($reformasiRb->file_link)
                                <div class="mb-2 text-xs text-blue-600 dark:text-blue-400">
                                    Link saat ini: <a href="{{ $reformasiRb->file_link }}" target="_blank" class="underline hover:text-blue-800 dark:hover:text-blue-300">Buka Link</a>
                                </div>
                            @endif
                            <input type="url" name="file_link" value="{{ old('file_link', $reformasiRb->file_link) }}" 
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                            @error('file_link') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                        </div>

                        {{-- Cover Image --}}
                        <div x-data="{ 
                            preview: '{{ $reformasiRb->cover_image ? Storage::url($reformasiRb->cover_image) : null }}',
                            handleFile(event) {
                    ...
                    <div>
                        ...
                        <input id="description" type="hidden" name="description" value="{{ old('description', $reformasiRb->description) }}">
                    ...
                    <div class="flex items-center">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $reformasiRb->is_active) ? 'checked' : '' }}>
                ...
                <div class="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
                    <a href="{{ route('dashboard.reformasi-rb.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-colors">Batal</a>
