<x-dashboard-layout>
    <x-slot:title>Manajemen Reformasi RB</x-slot:title>
    ...
    class="pb-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6 pt-2">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Kelola data dan dokumen terkait Reformasi Birokrasi.
                </p>
            </div>
            
            {{-- Tombol Aksi Header --}}
            <a href="{{ route('dashboard.reformasi-rb.create') }}" 
        ...
        <form method="GET" action="{{ route('dashboard.reformasi-rb.index') }}" class="mb-6">
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'sort']))
                        <a href="{{ route('dashboard.reformasi-rb.index') }}" class="w-full md:w-auto px-5 py-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 text-center transition-colors">Reset</a>
                    @endif
                </div>
            </div>
        </form>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-300">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-center w-24">Cover</th>
                            <th scope="col" class="px-4 py-3 text-left min-w-[200px]">Judul & Link</th>
                            <th scope="col" class="px-4 py-3 text-left min-w-[250px]">Keterangan</th>
                            ...
                            <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        ...
                                        @if($item->file_link)
                                            <a href="{{ $item->file_link }}" target="_blank" rel="noopener noreferrer" 
                                               class="inline-flex items-center gap-1 text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                                <svg class="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
                                                Buka File/Link
                                            </a>
                                        @endif
                                    </div>
                                ...
                                <td class="px-4 py-4 align-top text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('dashboard.reformasi-rb.edit', $item->id) }}" 
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm mt-1">Silakan tambahkan data Reformasi RB baru.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div x-show="deleteModal" 
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full overflow-hidden" @click.away="closeDeleteModal()">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                Data <span class="font-medium text-gray-900 dark:text-white" x-text="deleteItemTitle"></span> akan dihapus permanen.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <form :action="`{{ route('dashboard.reformasi-rb.destroy', '') }}/${deleteItemId}`" method="POST">
                        @csrf
                        @method('DELETE')
