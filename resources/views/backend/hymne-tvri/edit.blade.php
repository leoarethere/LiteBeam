<x-dashboard-layout>
    <x-slot:title>Edit Himne TVRI</x-slot:title>
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-8 pt-2">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Perbarui data lagu atau himne.
                </p>
            </div>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            <form action="{{ route('dashboard.hymne-tvri.update', $hymneTvri->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $hymneTvri->title) }}" 
                        ...
                        <div>
                            ...
                            <input type="text" name="info" id="info" value="{{ old('info', $hymneTvri->info) }}" 
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                            @error('info') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                        </div>

                        {{-- Link Media --}}
                        <div>
                            <label for="link" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Link Video/Audio (URL) <span class="text-red-500">*</span>
                            </label>
                            <input type="url" name="link" id="link" value="{{ old('link', $hymneTvri->link) }}" 
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                    ...
                    <div>
                        <label for="synopsis" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Sinopsis / Lirik <span class="text-red-500">*</span>
                        </label>
                        <input id="synopsis" type="hidden" name="synopsis" value="{{ old('synopsis', $hymneTvri->synopsis) }}">
                        <trix-editor input="synopsis" class="trix-content bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white min-h-[250px]"></trix-editor>
                        @error('synopsis') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                    </div>

                    {{-- Poster (dengan Alpine Preview) --}}
                    <div x-data="{ 
                        preview: '{{ $hymneTvri->poster ? Storage::url($hymneTvri->poster) : null }}',
                        handleFile(event) {
                        ...
                        <input type="file" name="poster" id="poster" @change="handleFile($event)" 
                    ...
                    <div class="flex items-center">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $hymneTvri->is_active) ? 'checked' : '' }}>
                ...
                <div class="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
                    <a href="{{ route('dashboard.hymne-tvri.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-colors">
                        Batal
                    </a>
