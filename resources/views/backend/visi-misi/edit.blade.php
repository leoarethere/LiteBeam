<x-dashboard-layout>
    <x-slot:title>Edit Visi/Misi</x-slot:title>

    {{-- Style Trix (Sama) --}}
    <link rel="stylesheet" type="text/css" href="https://unpkg.com/trix@2.0.8/dist/trix.css">
    <style>
        /* ... (CSS Trix sama seperti sebelumnya) ... */
        ...
        .dark trix-editor blockquote { border-left-color: #4b5563; color: #d1d5db; }
    </style>
    ...
    <div class="pb-6">
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-8 pt-2">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui informasi Visi atau Misi.</p>
            </div>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            {{-- [BARU] Tambahkan enctype --}}
            <form action="{{ route('dashboard.visi-misi.update', $visiMisi) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="visi" @selected($visiMisi->type == 'visi')>Visi (Utama)</option>
                            <option value="misi" @selected($visiMisi->type == 'misi')>Misi (Poin)</option>
                        </select>
                    </div>

                    {{-- [BARU] Input Gambar dengan Preview (Menampilkan gambar lama jika ada) --}}
                    <div x-data="{ preview: '{{ $visiMisi->image ? Storage::url($visiMisi->image) : null }}' }">
                        ...
                        <input type="file" name="image" x-ref="fileInput" @change="preview = URL.createObjectURL($event.target.files[0])" accept="image/png, image/jpeg, image/jpg, image/webp" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400">
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Ganti gambar jika ingin mengubahnya.</p>
                        @error('image')
                    ...
                    <div>
                        ...
                        <input id="content" type="hidden" name="content" value="{{ old('content', $visiMisi->content) }}">
                        <trix-editor input="content" class="trix-content bg-gray-50 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"></trix-editor>
                        @error('content')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="order" value="{{ $visiMisi->order }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        </div>
                        <div class="flex items-center pt-8">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" class="sr-only peer" @checked($visiMisi->is_active)>
                ...
                <div class="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
                    <a href="{{ route('dashboard.visi-misi.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-colors">Batal</a>
