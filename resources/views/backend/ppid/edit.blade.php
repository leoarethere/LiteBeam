    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">Edit Data PPID</h2>
            </div>

            <form action="{{ route('dashboard.ppid.update', $ppid->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $ppid->title) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                File Sumber (PDF/DOC/XLS)
                            </label>
                            @if($ppid->file_path)
                                <div class="mb-2 text-xs text-blue-600 dark:text-blue-400">
                                    File saat ini: <a href="{{ asset('storage/' . $ppid->file_path) }}" target="_blank" class="underline hover:text-blue-800 dark:hover:text-blue-300">Lihat File</a>
                                </div>
                            @endif
                            <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" 
                                name="file_path" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Biarkan kosong jika tidak ingin mengganti file.</p>
                            @error('file_path') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if($ppid->cover_image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $ppid->cover_image) }}" alt="Preview" class="h-20 w-auto rounded border border-gray-200 dark:border-gray-600">
                                </div>
                            @endif
                            <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" 
                    ...
                    <div>
                        ...
                        <input id="description" type="hidden" name="description" value="{{ old('description', $ppid->description) }}">
                ...
                <div class="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
                    ...
                    <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 transition-colors">Perbarui Data</button>
                </div>
            </form>
        </div>
    </div>
