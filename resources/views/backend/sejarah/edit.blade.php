        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            <form action="{{ route('dashboard.sejarah.update', $history) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-6">
                    ...
                    {{-- Gambar Ilustrasi --}}
                    <div x-data="{ preview: '{{ $history->image ? Storage::url($history->image) : null }}' }">
                        ...
                        <input type="file" name="image" id="image" x-ref="fileInput" @change="preview = URL.createObjectURL($event.target.files[0])" accept="image/png, image/jpeg, image/jpg, image/webp" class="block w-full text-sm text-gray-900 border rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 {{ $errors->has('image') ? 'border-red-500' : 'border-gray-300' }}">
                        @error('image')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Pilih gambar baru untuk mengganti yang lama (Max 5MB)
                        </p>
                    </div>
                </div>
