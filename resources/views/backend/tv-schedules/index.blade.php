<x-dashboard-layout>
    <x-slot:title>Manajemen Jadwal TV</x-slot:title>
    ...
    class="pb-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-8 pt-2">
            ...
            <a href="{{ route('dashboard.tv-schedules.create') }}" 
                        ...
                        @forelse ($schedules as $index => $item)
                            <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4 text-xs">
                                    @if($item->linkSource)
                                        <span class="px-2 py-1 bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 rounded border border-gray-200 dark:border-gray-600">
                                            {{ $item->linkSource->name }}
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        {{-- Edit --}}
                                        <a href="{{ route('dashboard.tv-schedules.edit', $item) }}" 
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    Belum ada data jadwal. Silakan tambahkan baru.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 max-w-md w-full" @click.away="closeDeleteModal()">
                <div class="p-6 text-center">
                    ...
                    <div class="flex justify-center gap-4">
                        ...
                        <form :action="`{{ route('dashboard.tv-schedules.destroy', '') }}/${deleteItemId}`" method="POST">
                            @csrf
                            @method('DELETE')
