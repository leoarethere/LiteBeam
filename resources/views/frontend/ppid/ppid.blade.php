<x-layout>
    <x-slot:title>Pejabat Pengelola Informasi dan Dokumentasi</x-slot:title>

    {{-- KONTAINER UTAMA --}}
    <div class="px-4 sm:px-6 lg:px-8">

        {{-- HERO SECTION (Desain Header Konsisten) --}}
            ...
            <div class="relative px-6 py-12 lg:px-12 lg:py-20 text-center">
                <div class="max-w-3xl mx-auto">
                    ...
                    <p class="text-blue-100 text-lg max-w-2xl mx-auto leading-relaxed">
                        Pejabat Pengelola Informasi dan Dokumentasi (PPID) TVRI Stasiun Yogyakarta hadir untuk menjamin keterbukaan informasi publik dengan layanan yang transparan, cepat, dan mudah diakses masyarakat.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6">
                ...
                <div class="flex flex-col md:flex-row gap-4 p-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['search', 'sort']))
                            <a href="{{ route('ppid.index') }}" class="w-full md:w-auto px-6 py-3 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-colors flex items-center justify-center">
                                Reset
                            </a>
                        @endif
                    </div>
            ...
            @if(request()->hasAny(['search', 'sort']))
                <div class="max-w-5xl mx-auto mt-4 text-sm text-gray-600 dark:text-gray-400">
                    Menampilkan {{ $ppids->total() }} dokumen
                    @if(request('search'))
        ...
        <div class="mb-16">
            ...
            @else
                ...
                <div class="text-center py-20 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Dokumen Tidak Ditemukan</h3>
                    <p class="text-gray-600 dark:text-gray-400 max-w-md mx-auto mb-8">
                        @if(request('search'))
                            Tidak ada dokumen yang cocok dengan kata kunci <strong>"{{ request('search') }}"</strong>.
                        @else
                            Belum ada dokumen publik yang tersedia saat ini.
                        @endif
                    </p>
                    @if(request('search'))
                        <a href="{{ route('ppid.index') }}" class="inline-flex items-center px-5 py-2.5 border border-transparent text-sm font-medium rounded-xl text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-sm">
                            Tampilkan Semua
                        </a>
                    @endif
                </div>
            ...
            @if($ppids->hasPages())
                <div class="mt-8 flex justify-center">
                    {{ $ppids->withQueryString()->links('vendor.pagination.tailwind') }}
                </div>
            @endif
        </div>
    </div>
