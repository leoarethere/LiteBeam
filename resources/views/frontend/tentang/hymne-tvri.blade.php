<x-layout>
    ...
    @push('styles')
        ...
        <style>
            ...
            /* CSS 3D Effect (Konsisten dengan Sejarah, Prestasi, dll) */
            /* Default Mobile: Flat */
            .hymne-card-3d-left, 
            .hymne-card-3d-right {
                transform: none;
            ...
            @media (min-width: 768px) {
                ...
                /* Hover: Kembali Flat (Penting untuk menonton video) */
        ...
        <div class="space-y-12 md:space-y-16">
            @forelse($hymnes as $hymne)
                @php
                    ...
                    // 2. Logika Layout Zig-Zag (Ganjil/Genap)
                    $isOdd = $loop->odd;
                    ...
                    // Order Desktop: Ganjil (Normal), Genap (Swap)
                ...
                <div class="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-0">
                        ...
                        {{-- Mobile: Urutan HTML pertama (selalu di atas). Desktop: Diatur via order class --}}
                        <div class="md:col-span-2 relative aspect-video md:aspect-auto md:min-h-[350px] overflow-visible p-6 sm:p-8 flex items-center justify-center {{ $bgGradient }} {{ $orderMedia }}">
                            ...
                            <div class="relative w-full" style="perspective: 1000px;">
                                <div class="relative w-full aspect-video bg-black rounded-xl shadow-2xl overflow-hidden {{ $class3D }}">
                                    ...
                                    {{-- B. LOCAL VIDEO --}}
                                    @elseif($hymne->link && (Str::endsWith($hymne->link, '.mp4') || Str::endsWith($hymne->link, '.m3u8')))
                                        <video id="hymne-video-{{ $hymne->id }}" 
                                            ...
                                            <source src="{{ $hymne->link }}" type="{{ Str::endsWith($hymne->link, '.m3u8') ? 'application/x-mpegURL' : 'video/mp4' }}">
                                        </video>
                        ...
                        <div class="md:col-span-3 p-6 sm:p-8 lg:p-12 flex flex-col justify-center bg-white dark:bg-gray-800 {{ $orderText }}">
                            ...
                            {{-- Tombol Link Eksternal (Jika bukan embed video langsung) --}}
                            @if($hymne->link && !$videoId && !Str::endsWith($hymne->link, ['.mp4', '.m3u8']))
